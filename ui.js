...
// ─── Cache SVG categorie ────────────────────────────────────────────────────
// Gli SVG vengono parsati UNA SOLA VOLTA e poi clonati, invece di chiamare
// DOMParser ad ogni render (che causava rallentamenti visibili con liste lunghe).
const _svgCache = new Map();

function getCategoryIconNode(categoryName) {
  if (_svgCache.has(categoryName)) {
    return _svgCache.get(categoryName).cloneNode(true);
  }
  const svgStr = getCategoryIconSvgString(categoryName);
  const doc = new DOMParser().parseFromString(svgStr, "image/svg+xml");
  const node = doc.documentElement;
  _svgCache.set(categoryName, node);
...
export function renderCategoryFilters() {
  ...
  if (!allInventoryCategories.has("Altro")) categoryCounts["Altro"] = 0;

  let totalAllCategoriesRemaining = 0;

  Object.values(state.currentChecklistState).forEach((stateEntry) => {
    if (stateEntry.itemData && !stateEntry.checked && !stateEntry.isExceptionallyShown) {
      const item = stateEntry.itemData;
      const itemCategory =
        item.categoria && item.categoria.trim() !== "" ? item.categoria : "Altro";
  ...
  // Fragment per un singolo reflow
  const fragment = document.createDocumentFragment();

  // Voce "Tutte le categorie"
  ...
  if (state.currentActiveCategoryFilter === null) allItemsFilter.classList.add("active");
  allItemsFilter.style.backgroundColor = CATEGORY_COLORS.Default;
  allItemsFilter.appendChild(getCategoryIconNode("Tutte le categorie"));
  ...
  fragment.appendChild(allItemsFilter);
  ...
  if (allInventoryCategories.has("Altro") && !sortedUniqueCategories.includes("Altro"))
  ...
  sortedUniqueCategories.forEach((category) => {
    ...
    if (state.currentActiveCategoryFilter === category && remainingInCategory === 0)
    ...
    if (remainingInCategory > 0 || state.currentActiveCategoryFilter === category) {
      ...
      if (state.currentActiveCategoryFilter === category) filterItem.classList.add("active");
      filterItem.style.backgroundColor = CATEGORY_COLORS[category] || CATEGORY_COLORS.Default;
      filterItem.appendChild(getCategoryIconNode(category));
  ...
  dom.sidebarCategoryFiltersContainer.innerHTML = "";
  dom.sidebarCategoryFiltersContainer.appendChild(fragment);

  return activeCategoryIsEmpty;
}

// ─── Margine contenuto principale ──────────────────────────────────────────

export function adjustMainContentMargin() {
  const targetMarginLeft = dom.sidebarMenu.classList.contains("open") ? "300px" : "70px";
  if (dom.initialSectionsWrapper && !dom.initialSectionsWrapper.classList.contains("hidden"))
...
function countActuallyFilteredItemsRecursive(items, selectedUtilizzi, activeCategory = null) {
  let totalFiltered = 0;
    ...
    itemList.forEach((subItem) => {
      const passesUtilizzo =
        selectedUtilizzi && selectedUtilizzi.length > 0
          ...
          : true;
      const passesCategory = !activeCategory || subItem.categoria === activeCategory;
      if (passesUtilizzo && passesCategory) totalFiltered++;
      if (subItem.children) recursiveCount(subItem.children);
    });
  }
...
export function updateCounters() {
  ...
  const actualFilteredCountForDisplay = countActuallyFilteredItemsRecursive(
    state.fullInventoryData.inventario,
    ...
    if (entry.checked && entry.itemData && !entry.isExceptionallyShown) {
      const passesEffectiveUtilizzo = state.currentChecklistState.original_filters
        ? state.currentChecklistState.original_filters.length === 0 ||
          state.currentChecklistState.original_filters.includes(entry.itemData.utilizzo)
        : currentSelectedUtilizzi.length === 0 ||
          currentSelectedUtilizzi.includes(entry.itemData.utilizzo);
      const passesCategoryFilter =
        !state.currentActiveCategoryFilter ||
        entry.itemData.categoria === state.currentActiveCategoryFilter;
      if (passesEffectiveUtilizzo && passesCategoryFilter) visibleCheckedItemsCount++;
    }
  });
  dom.sidebarCheckedElementsSpan.textContent = visibleCheckedItemsCount;
  dom.sidebarRemainingElementsSpan.textContent = Math.max(
    0,
  ...
  if (state.fullInventoryData && state.fullInventoryData.inventario) {
    Object.values(state.currentChecklistState).forEach((stateEntry) => {
      if (stateEntry.itemData && !stateEntry.isExceptionallyShown) {
        const passes =
          currentSelectedUtilizzi.length === 0 ||
          currentSelectedUtilizzi.includes(stateEntry.itemData.utilizzo);
        if (passes) {
          total++;
          if (stateEntry.checked) checked++;
        }
      }
    });
  }
...
function updateUtilizzoCounters(inventory) {
  ...
  if (tagsHtml) {
    dom.sidebarInventoryUtilizzoCounters.innerHTML = "Utilizzo Inventario: " + tagsHtml;
...
function itemOrDescendantPassesFilter(item, selectedUtilizzi, activeCategory = null) {
  const passesUtilizzo =
    !selectedUtilizzi || selectedUtilizzi.length === 0 || selectedUtilizzi.includes(item.utilizzo);
...
// Su iPad/iOS il checkbox nativo ha un ritardo di ~300ms sull'evento "change".
// Intercettiamo touchend con preventDefault per rispondere immediatamente,
// ignorando i movimenti di scorrimento (delta > 10px).

function attachCheckboxEvents(checkbox, uniqueId) {
  ...
  checkbox.addEventListener("touchend", (e) => {
    ...
    touchStartY = null;
    touchStartX = null;
    // Se l'utente stava scrollando, non fare nulla
    if (dy > 10 || dx > 10) return;
    e.preventDefault(); // Elimina il click sintetico ritardato di iOS
    const newChecked = !checkbox.checked;
    checkbox.checked = newChecked;
    state.setHasUnsavedChanges(true);
    handleItemCheck(uniqueId, newChecked);
  });

  // Fallback per mouse e tastiera (desktop)
...
function createChecklistItem(
  ...
  if (isCurrentItemExceptionallyShown) li.classList.add("item-exceptionally-shown");
  ...
  if (!state.currentChecklistState[uniqueId]) {
    ...
    state.currentChecklistState[uniqueId].isExceptionallyShown = isCurrentItemExceptionallyShown;
  }

  // Tre casi distinti:
  // "parent-with-children" → ha figli nell'inventario
  // "root-leaf"            → è root (nessun parentId) ma senza figli
  // "child"                → ha un parentUniqueId (è annidato)
  const itemTypeClass = isPotentialParent
    ? "parent-with-children"
    : parentUniqueId
      ? "child"
      : "root-leaf";

  li.classList.add(`item--${itemTypeClass}`);

  const itemRowDiv = document.createElement("div");
  itemRowDiv.classList.add("item-row");
  itemRowDiv.classList.add(`item-row--${itemTypeClass}`);
  ...
  spacer.classList.add(`toggle-spacer--${itemTypeClass}`);
  ...
  if (!isCurrentItemExceptionallyShown) {
    attachCheckboxEvents(checkbox, uniqueId);
  }
  ...
  if (item.children && item.children.length > 0) {
    ...
    // Fragment per i figli: un solo reflow per tutti
    ...
    sortedChildren.forEach((child) => {
      ...
      const childPassesCategory =
        !activeCategoryForRendering || child.categoria === activeCategoryForRendering;
      const childPassesAllCurrentFilters = childPassesUtilizzo && childPassesCategory;
      ...
      if (childOrDescendantPassesAll) {
        ...
        childrenFragment.appendChild(childListItem);
      }
    });
...
export function renderChecklist() {
  ...
  const selectedUtilizziForRendering = state.currentChecklistState.original_filters || [];
  ...
  // Garantisce che lo stato esista per tutti gli item prima di rendere
  function ensureStateExistsRecursive(items, parentId = null) {
    items.forEach((item) => {
      ...
      if (item.children) ensureStateExistsRecursive(item.children, itemId);
    });
  }
  ...
  // Accumula tutto in un fragment → un solo reflow DOM alla fine
  const fragment = document.createDocumentFragment();
  ...
  sortedInventory.forEach((item) => {
    ...
    const itemPassesAllCurrentFilters = itemPassesUtilizzo && itemPassesCategory;
...
// ─── Aggiornamento tipo visivo item ────────────────────────────────────────
...
function updateItemVisualType(uniqueId) {
  ...
  if (itemState.itemData.children && itemState.itemData.children.length > 0 && childrenUl) {
...
export function applyVisualState(uniqueId, listItemElement = null) {
  ...
  if (itemState.checked && !itemState.isExceptionallyShown) {
    if (itemState.itemData.children && itemState.itemData.children.length > 0) {
      const childrenUl = listItem.querySelector(".children-list");
      ...
      if (
        ...
        if (visibleChildrenInUl.length > 0) {
          allChildrenEffectivelyChecked = visibleChildrenInUl.every((childLi) => {
            const childState = state.currentChecklistState[childLi.dataset.itemId];
            return (
              childState &&
...
export function updateParentState(parentUniqueId) {
  ...
  const parentListItem = document.querySelector(`[data-item-id="${parentUniqueId}"]`);
  applyVisualState(parentUniqueId, parentListItem);
  const parentState = state.currentChecklistState[parentUniqueId];
  if (parentState && parentState.parentId) updateParentState(parentState.parentId);
}

/**
 * Versione ottimizzata: aggiorna prima i parent (che hanno figli), poi i leaf.
 * Evita di iterare due volte su tutti gli item con querySelector generici.
 */
export function updateAllParentStates() {
  const parentIds = new Set();
  Object.entries(state.currentChecklistState).forEach(([id, entry]) => {
    if (
      entry &&
      typeof entry === "object" &&
      entry.itemData &&
      entry.itemData.children &&
      entry.itemData.children.length > 0
    ) {
      parentIds.add(id);
    }
  });

  // Prima i parent (che dipendono dallo stato dei figli)
  parentIds.forEach((uniqueId) => applyVisualState(uniqueId));

  // Poi i leaf (che potrebbero dipendere da aggiornamenti ai parent)
  Object.keys(state.currentChecklistState).forEach((uniqueId) => {
    if (!parentIds.has(uniqueId)) applyVisualState(uniqueId);
  });
}
...
export function updateCheckedItemsOverlay() {
  ...
  for (const uniqueId in state.currentChecklistState) {
    const itemState = state.currentChecklistState[uniqueId];
    if (itemState.checked && itemState.itemData && !itemState.isExceptionallyShown) {
  ...
  checkedItems.forEach((itemStateWithId) => {
    const li = document.createElement("li");
    const displayName = itemStateWithId.itemData.nome || itemStateWithId.itemData.id;
    let countPrefix = "";
    ...
    if (
      ...
      countPrefix =
        itemData.totali === itemData.in_uso
          ? `<span class="history-count">${itemData.totali}</span> `
          : `<span class="history-count">${itemData.in_uso}/${itemData.totali}</span> `;
    }
    ...
    if (
      ...
      const parentData = state.currentChecklistState[itemStateWithId.parentId].itemData;
      const parentName = parentData.nome || parentData.id;
      finalDisplayNameHTML = `${countPrefix}${displayName} <span class="history-parent">(${parentName})</span>`;
    } else {
      finalDisplayNameHTML = `${countPrefix}${displayName}`;
    }
  ...
  dom.checkedItemsList.appendChild(fragment);
}
